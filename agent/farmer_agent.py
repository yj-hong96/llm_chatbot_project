...
[지침]
1.  **상위 의도 파악**: 먼저 사용자의 최신 질문이 '새로운 작물 추천'을 원하는 것인지, 아니면 이전에 언급된 작물에 대한 '상세 설명'을 원하는 것인지 파악하세요.

2.  **'상세 설명' 요청 처리**:
    - 만약 질문이 '상세 설명'에 해당한다면, 질문의 구체적인 주제가 '재배 방법', '병해충 관리', '농약 정보' 등 어떤 농업 주제에 관한 것인지 파악하세요.
    - 그 다음, [대화 기록]에서 언급된 모든 작물 이름(예: 비트, 양파, 상추)을 찾아내세요.
    - 마지막으로, 파악한 주제와 작물 이름을 결합하여 "비트 재배 방법 또는 양파 재배 방법", "상추 병해충 관리" 와 같이 구체적인 검색 질문들을 생성하고, '또는'으로 연결하여 하나의 긴 질문으로 만드세요.

3.  **'새로운 추천' 요청 처리**:
    - 만약 질문이 '새로운 추천'에 해당한다면, [대화 기록]에서 사용자가 제시한 핵심 조건(예: '고랭지 지역', '습한 토양')을 찾아내세요.
    - 그 조건과 사용자의 최신 질문에 언급된 새로운 작물(예: '감자')을 결합하여 "고랭지 지역이고 습한 토양에 감자를 포함하여 추천"과 같이 하나의 포괄적인 검색 질문을 만드세요.

4.  **출력 형식 엄수**: 최종 결과는 오직 재작성된 '검색용 질문' 한 줄이어야 합니다. 부가적인 설명이나 다른 텍스트를 절대 포함하지 마세요.

[예시 1: 상세 설명 요청]
- 이전 대화: 챗봇: ...비트, 양파를 추천합니다.
- 사용자 질문: 어떻게 키워야 해?
- 재작성된 검색용 질문: 비트 재배 방법 또는 양파 재배 방법
...
3.  **'설명' 답변**: 만약 사용자가 재배 방법 등을 물었다면, [검색된 참고 정보]에서 각 작물의 재배 방법에 대한 내용을 찾아 명확하게 구분하여 설명해주세요.
...
if __name__ == "__main__":
    try:
        graph_image_path = "farer_graph.png"
